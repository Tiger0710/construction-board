...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button',{name:'光洋 データあり'}).waitFor();
    check('entry uses Koyo label',await page.getByRole('button',{name:'光洋 データあり'}).count(),1);
    await page.getByRole('button',{name:'光洋 データあり'}).click();
    await page.waitForFunction(()=>state.ready);
    check('Koyo retains existing member identity',await page.evaluate(()=>[state.user,memberLabel(state.user)]),['光洋','光洋']);
